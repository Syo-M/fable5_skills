...
- **レビュアーエージェントの「読み取り専用」** — v3.0.1 から**ツールレベルでも強制**されます:
  エージェント固有の PreToolUse フック(`reviewer-write-guard.mjs`、テスト14ケース同梱)が
  リポジトリへの Edit/Write と書き込み系 Bash を **deny** します(エージェントメモリへの書込みのみ許可)。
  **要件: Claude Code CLI ≥ 2.1.145**。それ未満の CLI とプラグイン経由のエージェントでは
  agent フックが無視されるため、従来どおり指示レベルの契約にフォールバックします(正直に明記)。
...
- **発動実測(v3.0.0)**: デフォルトプロファイル 29本×3ラン — フルPASS **24/29**・ラン単位 **77/87 = 88.5%**。
  ...
  オーバートリガー副作用は実測ゼロ
- **第三者評価(参考)**: ChatGPT による独立レビューで **89 / 100**(v2.0.0 時点。その P0 指摘は v2.1.0 で対応済み)
