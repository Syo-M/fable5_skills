...
```bash
./install.sh /path/to/your-project          # 既存ファイルはスキップ(ローカル編集を保護)
...
`CLAUDE.md.fable-skills` を併置、導入バージョンを `.claude/fable-skills-version` に記録
(どのルールがどのコミットを統治したか監査可能に)。
...
**v2.0.0 の最終実測(2026-07-02):**

| ペルソナ | v1.9.0 | v2.0.0候補 | **v2.0.0最終** | 判定 |
|---|---|---|---|---|
| Staff Frontend Engineer | 88.1 | 88.6 | **90.4** | 内容修正なしで全社採用可 |
| Security & Platform Lead | 89.4 | 89.7 | **91.5** | サインオフ(無条件) |
| LLMプロンプト設計者 | 89.3 | 89.6 | **91.3** | **state-of-the-art 認定** |
| **平均** | 88.9 | 89.3 | **91.1** | |

「候補」→「最終」の差は、3採点者が指名した反証実験(`--max-turns 6` 感度実験)の実施と
収束指摘の即時修正によるものです(証拠で上げた点数であり、再交渉ではありません。各採点者は
同一人格・同一較正のまま差分を実行検証して再レートしています)。

**バージョン推移(平均):** v1.1.0 84.5 → v1.2.0 86.9 → v1.3.0 88.8 → v1.5.0 87.7 →
v1.6.0候補 88.2 → v1.9.0 88.9 → **v2.0.0 91.1**

**v2.0.0 をメジャーとした根拠**: `MAINTENANCE.md` が義務付ける MAJOR 前ゲート
(`--runs 3` リリースグレード発動評価+ペルソナ採点)を両方満たした**計測されたマイルストーン**
です(v1.9.1 時点では同ゲート未実施のため3者一致で延期していました)。破壊的変更はありません。
...
### 発動実測(トリガー評価)— リリースグレード `--runs 3` 済み

「ルールが書いてある」と「発動する」は別の主張なので、後者を `eval/` ハーネスで**フックによる
観測**で実測しています(推測や自己申告ではない)。

**v2.0.0 正式測定(29本 × 3ラン = 87ラン、全て有効、2026-07-02):**

- **フルPASS(3/3): 23/29(79%) / ラン単位発動率: 74/87 = 85%**
  (参考比較: v1.8.0 ベースラインは26本×1ランで 12/26 = 46%。同一26本での対比は 20/26 = 77%。
  改善の主因は CLAUDE.md の load-first 指示)
- **オーバートリガー負例 3本すべて 3/3 クリーン** — 「常にロードせよ」指示による過剰ロードの
  副作用は実測ゼロ
- エージェント単独期待の5本はすべて 3/3(完全)。レイヤー別信頼性:
  **エージェント > パスルール > 手順スキル > 領域ルールブックスキル**
- **3ターンで 0/3 だった3本は、`--max-turns 6` の感度実験で全て 3/3 に転換**
  (`security-form-jp`→frontend-security、`chart-en`→data-viz、`react-state-jp`→react-patterns が
  ターン4〜6でロード)。つまり「発動しない」のではなく**3ターンの測定地平より後で発動する**ことを
  実証 — 現実的なターン数では**ゴールデンプロンプト29本すべてが発動を実証済み**
  (実験: `eval/reports/v2.0.0-horizon-experiment.md`。3ターン値 23/29・85% は保守的な見出し値
  として維持)
- 経緯(v1.9.0 ベースライン 12/26 → 原因特定 → 修正の実測検証)は
  `eval/reports/v1.9.0-summary.md`、手法は `eval/README.md`
