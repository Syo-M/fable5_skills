...
**v1.9.0 の実測(2026-07-02、最新):**

| ペルソナ | v1.6.0候補 | v1.9.0 | Δ | 判定 |
|---|---|---|---|---|
| Staff Frontend Engineer | 86.7 | **88.1** | +1.4 | 修正条件付き採用(チーム展開は即GO) |
| Security & Platform Lead | 89.0 | **89.4** | +0.4 | 条件付きサインオフ(条件は1行修正級) |
| LLMプロンプト設計者 | 89.0 | **89.3** | +0.3 | SOTA目前(残ギャップは測定報告の厳密性) |
| **平均** | **88.2** | **88.9** | **+0.7** | |

**バージョン推移(平均):** v1.1.0 84.5 → v1.2.0 86.9 → v1.3.0 88.8 → v1.5.0 87.7 →
v1.6.0候補 88.2 → **v1.9.0 88.9**
(v1.5.2〜v1.5.6 は別ラウンドの Semgrep/CI 強化修正。v1.9.0 採点の収束指摘 — `oauth2/` の
フック正規表現すり抜け、eval 統計の表現厳密化、実行妥当性ガード等 — は **v1.9.1 で修正済み**)

**v2.0.0 を見送った理由(3者一致)**: `MAINTENANCE.md` が MAJOR 前に義務付ける `--runs 3`
発動評価が未実施であり、自らのリリースプロトコルに従い延期。追加はすべて後方互換(意味論的に
MINOR)。90超えは「主張」ではなく「実測」で達成する方針です。
...
観測**で実測しています(推測や自己申告ではない)。ゴールデンプロンプト26本(日英)の結果:

- ベースライン **12/26** → 原因分析(モデルが"直接できる作業"でルールブックスキルのロードを
  省略する)→ CLAUDE.md に「スキルロードは任意ではない」を明記 → **再実測で失敗14本中9本が
  PASS 転換**、有効測定の合算で **22/26 が発動を実証**
- レイヤー別の発動信頼性: **エージェント > パスルール > 手順スキル > 領域ルールブックスキル**
- 恒常的な未発動4本は既知ギャップとして記録(該当ケースでもルール層/CLAUDE.md 床が不変条件を
  カバー)。詳細と手法は `eval/README.md`、統合レポートは `eval/reports/v1.9.0-summary.md`
