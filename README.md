...
**上記スタックを前提とした意見の強い(opinionated)標準セット**です。Tailwind / MUI /
styled-components / Jest / Cypress / Vue / Svelte が前提の現場では、そのままでは衝突します
(スタイリング規約は v3.0 で選択式プロファイル化を計画 — 単一リポジトリ + `profiles/` 方式)。
ライセンスは **MIT** です(`LICENSE`)。
...
設計し、正式評価で3ターン以内のラン単位発動率85%を実測(全29プロンプトは最大6ターン以内で
発動を確認 — 詳細は `EVALUATION.md`)。
...
./install.sh /path/to/your-project --dry-run  # 何が起きるかの計画だけ表示(変更なし)
./install.sh /path/to/your-project --check    # 導入済みバージョンと差分ファイル数を表示
...
- **内部評価(v2.0.0)**: 3ペルソナ加重採点で **91.1 / 100**(Engineer 90.4 / Security 91.5 / LLM 91.3)
- **発動実測**: 29ゴールデンプロンプト × 3ラン、3ターン以内のラン単位発動率 **85%**
  (全29本は最大6ターン以内で発動を確認。オーバートリガー副作用は実測ゼロ)
- **第三者評価(参考)**: ChatGPT による独立レビューで **89 / 100**(その P0 指摘は v2.1.0 で対応済み)

> スコアの手法・ルーブリック・生データ・**自己評価としての限界**・再現手順はすべて
> **[`EVALUATION.md`](EVALUATION.md)** に集約しています(AI採点は人間のセキュリティレビューの
> 代替ではありません)。次回の正式採点は v3.0.0(profiles 化)の MAJOR ゲートとして実施します。
